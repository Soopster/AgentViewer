...
const METADATA_FILE = path.join(DATA_DIR, 'pi-metadata.json')
...
export async function getPiStoredMetadata(sessionId: string): Promise<{ title: string | null; tag: string | null }> {
  const store = await loadMetadataStore()
  return entryOrNull(store[sessionId])
}

export async function getPiStoredMetadataForSessions(
  sessionIds: string[],
...
export async function setPiStoredTitle(sessionId: string, title: string | null): Promise<void> {
...
export async function setPiStoredTag(sessionId: string, tag: string | null): Promise<void> {
  await updateMetadata(sessionId, (entry) => ({
    ...entry,
