...
const BASE_COORD_TOOL_SPECS = [
  {
    name: 'coord_wait',
    description: 'Wait for board changes only in an interactive host without resource subscriptions. Managed turns must never call this tool: return control when idle so the supervisor waits without model tokens. With agent, return only when that teammate settles (idle, ready, done, blocked, failed or stopped) or reaches an until state, or when mail needs your reply.',
    fields: {
  ...
  },
  {
    ...
    description: 'Add a task with dependencies and expected write paths to the shared board. Lead: set assign_to to delegate directly to an idle teammate by name or ID, reusing its session; the operation is atomic and returns queued delivery, not proof of execution. Optionally give it a role_name/role_description specialization for whoever claims it. The result includes `similarTasks` when this looks like it may duplicate existing work — not blocking, but check before assuming it\'s new.',
    fields: {
  ...
  {
    name: 'coord_delegate',
    description: 'Lead: ask another agent to do a concrete task. Omit to to reuse an idle teammate or create one in a server-managed run; with name (lowercase, e.g. reviewer), use the live teammate of that name or create one under it; name an agent in to for follow-ups. With wait_ms (interactive hosts only), also wait for the work to settle and return settled.outcome: completed, failed, cancelled, blocked, needs_reply, stalled (no activity seen yet — inspect before re-sending, never retry blindly) or timeout. Reuses its existing session for follow-up work. Atomically creates the task, grants its paths, and queues notification. Returns the task, agent, and session identity; queued is not proof of execution. Busy or conflicting assignments leave no task. Use coord_send_message to steer work already in progress.',
    fields: {
    ...
    action: 'create_task',
    mapArgs: (a) => ({ assignTo: a.to ?? 'auto', teammateName: a.to ? undefined : a.name, waitMs: a.wait_ms, title: a.title, detail: a.detail, paths: a.paths, verifyCommands: a.verify_commands, targetRole: 'teammate' }),
  },
  {
