...
const BASE_COORD_TOOL_SPECS = [
  ...
  {
    name: 'coord_delegate',
    description: 'Lead: ask another agent to do a concrete task. Omit to to reuse an idle teammate or create one in a server-managed run; with name (lowercase, e.g. reviewer), use the live teammate of that name or create one under it; name an agent in to for follow-ups. Reuses its existing session for follow-up work. Atomically creates the task, grants its paths, and queues notification. Returns the task, agent, and session identity; queued is not proof of execution. Busy or conflicting assignments leave no task. Use coord_send_message to steer work already in progress.',
    fields: {
    ...
    action: 'create_task',
    mapArgs: (a) => ({ assignTo: a.to ?? 'auto', teammateName: a.to ? undefined : a.name, title: a.title, detail: a.detail, paths: a.paths, verifyCommands: a.verify_commands, targetRole: 'teammate' }),
  },
  {
