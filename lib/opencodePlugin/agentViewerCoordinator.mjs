...
const TOOL_SPECS = [
  ...
  },
  {
    ...
    args: {
      ...
      requested_provider: z.enum(['codex', 'claude', 'copilot', 'opencode', 'pi']).optional(),
  ...
  },
  {
    ...
    description: 'Lead-only: approve or reject a completed phase gate.',
    args: { phase: z.string().min(1).max(120), approved: z.boolean(), summary: z.string().max(2000).optional(), detail: z.string().max(8000).optional() },
  },
  {
    name: 'coord_review_run',
    description: 'Lead-only: submit terminal judgment review after mechanical validation.',
    args: { approved: z.boolean(), summary: z.string().min(1).max(2000), detail: z.string().max(16000).optional() },
  },
  {
    name: 'coord_resolve_decision',
    description: 'Lead-only: answer or defer a structured open decision.',
    args: { task_id: z.string().min(1), decision_id: z.string().min(1), answer: z.string().min(1).max(4000), deferred: z.boolean().optional() },
  },
  {
    name: 'coord_promote_learning',
    description: 'Lead-only: record a recurring learning candidate for promotion.',
    args: { candidate_id: z.string().min(1), target: z.enum(['playbook', 'role', 'project_memory']) },
  },
  {
    ...
    args: {
      ...
      needs_decision_json: z.string().max(16000).optional(),
    },
  },
  {
