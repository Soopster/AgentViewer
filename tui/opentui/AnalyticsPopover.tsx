...
function buildInsights(a: Analytics): Insight[] {
  ...
  if (a.models.length > 1) {
    ...
    out.push({
      severity: 'info',
      icon: '⇄',
      title: `${a.models.length} models used`,
      detail: `Session switched between models: ${names}${a.models.length > 3 ? ` + ${a.models.length - 3} more` : ''}.`,
    })
  }

  // Cost acceleration — second half costs more than 1.8× first half
  if (a.cumulativeCost.length >= 6 && a.cost > 0) {
    ...
    if (firstHalf > 0 && secondHalf > firstHalf * 1.8) {
      out.push({
        severity: 'warn',
        icon: '↑',
        title: 'Cost accelerating',
        detail: `Second half of the session cost ${(secondHalf / firstHalf).toFixed(1)}x more than the first - context growing rapidly.`,
      })
    }
  }
  ...
  if (a.messageSizes.length >= 9) {
    ...
    if (medEarly > 0 && medLate > medEarly * 2) {
      out.push({
        ...
        detail: `Median message size grew ${(medLate / medEarly).toFixed(1)}x from early to late in the session (${fmtNum(medEarly)} -> ${fmtNum(medLate)} tokens).`,
      })
    }
  }
  ...
  if (a.toolErrors >= 3) {
    ...
    if (mostErrored && mostErrored.errors >= 2 && mostErrored.errors / a.toolErrors >= 0.5) {
      out.push({
        severity: 'warn',
        icon: '✕',
        title: `${mostErrored.name} error-prone`,
  ...
  if (ops.bashByVerb.size > 0) {
    ...
    if (repairCount >= 4 && repairCount > ops.bashCommands * 0.3) {
      out.push({
        severity: 'tip',
        icon: '⟳',
        title: 'Dependency/recovery work',
    ...
    if (latencies.length >= 6) {
      ...
      if (earlyMed > 0 && lateMed > earlyMed * 2 && lateMed > 10_000) {
        out.push({
          severity: 'warn',
          icon: '⤵',
          title: 'Responses slowing down',
          detail: `Later turns took ${(lateMed / earlyMed).toFixed(1)}x longer than early turns (${fmtDuration(earlyMed)} -> ${fmtDuration(lateMed)} median).`,
        })
      }
    }
  }
  ...
  if (a.assistantMessages > 0) {
    ...
    if (zeroOutputTurns >= 2 && zeroOutputTurns / a.assistantMessages > 0.15) {
      out.push({
        severity: 'info',
        icon: '◻',
        title: `${zeroOutputTurns} interrupted turns`,
