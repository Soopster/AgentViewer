...
try {
  ...
  releaseAuto!()
  await until(async () => !(await state()).delivery)
  await coord.appendProtocolEvent({ version: '1.0', runId, agentId: worker.agentId, type: 'message', to: 'lead', summary: 'Finished; no claimable tasks remain', payload: { kind: 'status' } })
  await coord.runProtocolMaintenanceSweep()
  assert.equal(autoTurns, 2, 'idle status after the result cannot spend another lead turn')
  ...
  const restarted = await post({ action: 'enable', requestId: 'new-team', detail: 'Start a new team', cwd })
  assert.notEqual(restarted.snapshot.run.id, runId)
  assert.equal(restarted.snapshot.tasks.length, 0)
  assert.equal(restarted.interactive.enabled, true)
  const restartedId = restarted.snapshot.run.id
  const retry = await post({ action: 'enable', requestId: 'new-team', detail: 'Start a new team', cwd })
  assert.equal(retry.snapshot.run.id, restartedId, 'enable retry must not create another team')
  const oldOff = await post({ action: 'disable', requestId: 'turn-off', detail: 'Turn off coordinator' })
  assert.equal(oldOff.snapshot.run.status, 'running', 'replaying old off must not stop the new team')
  assert.equal(oldOff.interactive.enabled, true)
  assert.equal((await coord.readProtocolRun(runId))?.run.status, 'stopped', 'old history remains stopped')
  await post({ action: 'settings', requestId: 'new-auto', detail: 'Enable continuation', autoContinue: true })
  await coord.stopProtocolRun(restartedId)
  const fresh = await post({ action: 'enable', requestId: 'after-direct-stop', detail: 'New team', cwd })
  assert.equal(fresh.interactive.autoContinue, false, 'new team must not inherit old continuation consent')
  assert.equal(fresh.interactive.remainingTurns, 4)
  await coord.stopProtocolRun(fresh.snapshot.run.id)
