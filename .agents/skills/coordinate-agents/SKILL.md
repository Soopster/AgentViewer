...
5. When no immediate action exists, follow the idle path for the current host:
   - **Managed `coord worker` turn:** if the turn prompt says to return control to a supervisor, do so immediately. Never call `coord_wait`, shell-sleep, or poll inside that model turn; the supervisor receives board changes and re-dispatches a fresh turn only when the `actionable` digest warrants one.
   - **Interactive host with resource subscriptions:** subscribe once to the `pushResource` returned by create/join/resume (`coord://agent-viewer/current-run`). On `notifications/resources/updated`, read that private resource and act on its authoritative `actionable` digest; the notification itself is only an invalidation.
   - **Interactive host without resource subscriptions:** call `coord_wait` with the previous cursor instead of repeatedly polling status. Your own writes do not wake it. On a Tasks-capable client, poll a returned task handle with `tasks/get` instead of issuing another wait. A timed-out result is normal; a thrown error is the real-disconnect case covered by the retry rule in "Enter the run" above.
6. After any wait, act on `actionable` and the returned events; the snapshot is authoritative if anything is unclear.

An interactive unsupervised participant must not end merely because the board is temporarily idle. A managed worker turn should return to its supervisor when idle. End participation when the run becomes `completed`, `failed`, or `stopped`; the user interrupts; or an external prerequisite cannot be resolved after notifying the lead.
