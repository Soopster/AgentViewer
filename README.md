...
MCP Tasks wrap long-running protocol operations; they do not replace Coordinator board tasks. Ownership, dependencies, path locks, mailboxes, findings, completion gates, and synthesis remain under the `coord_*` tools. Cancelling an MCP Task stops its wait or monitor, not the underlying multi-agent run. Polling is the supported delivery path for now; the optional `notifications/tasks` subscription is not implemented. The bridge carries a small compatibility adapter because the stable MCP TypeScript SDK v2 does not yet ship the experimental Tasks server runtime, and the smoke suite pins the published draft shapes to catch SDK drift.
