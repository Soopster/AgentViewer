...
Two optional MCP extensions make Coordinator workflows easier to discover and inspect:
...
The experimental Tasks extension (`io.modelcontextprotocol/tasks`) is intentionally not advertised yet. The current extension repository does not publish a server runtime, and the stable v2 TypeScript SDK rejects the older core `tasks/*` vocabulary on `2026-07-28` connections. Coordinator tasks remain durable through the existing `coord_*` tools until the extension has a supported implementation; advertising it without serving `tasks/get`, `tasks/update`, and `tasks/cancel` would violate capability negotiation.
